...
### 新規公開（4件、すべて verifiedAt: 2026-07-16）
...
- **「88日」対「3 months」:** `second-visa-guide` は特定就労要件を「88日（約3か月）」と表記。
  公式リスティングページの表現は "3 months"。「88日」は広く定着した通称だが、公式表現は月数。
  記事内では「88日（約3か月）」と併記し、齟齬を明示。**厳密な日数計算は公式・移民専門家に確認を促す。**
- **DASP の源泉税率（例: 65%）:** 帰国後スーパー返金（DASP）に適用される源泉税率は
  本サイクルでは公式照合を**未完了**。該当の断定的記述は行わず、`super-guide`/`dasp-refund` は
  一般的説明にとどめ、金額・税率の確定値は今後の照合対象（`BULK_PUBLISH_REMAINING.md`）。
